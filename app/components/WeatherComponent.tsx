...
export default function WeatherComponent() {
    ...
    useEffect(() => {
        getLocation().then(() => {
            fetchForecast(cityName as string, 7).then(data => {
                setWeatherData(data);
            }).finally(() => {
                setLoading(false);
            })
                ...
                <SafeAreaView className="flex flex-1">
                    <View className="mx-4 flex flex-1 justify-around mb-2">
                        ...
                        <View className="space-y-2">
                            <Text className="text-center text-white font-bold text-6xl">{current?.temp_c}°C</Text>
                            <Text className="text-center text-white tracking-widest text-xl">{current?.condition?.text}</Text>
                        </View>
                        ...
                        <View className="flex-row justify-between">
                            ...
                            <View className="space-x-2 flex-col items-center gap-1">
                                ...
                                <Text className="text-white font-semibold text-base">{current?.wind_kph} kph</Text>
                            </View>
                        </View>
                        ...
                        <View className="space-y-3 mb-2">
                            ...
                            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                                {weatherData?.forecast?.forecastday.map((forecast, index) => {
                                    ...
                                    return (
                                        <View key={index} className="space-y-1 mr-4 py-3 flex justify-center items-center w-24 rounded-md" style={{backgroundColor: 'rgba(255, 255, 255, 0.3)'}}>
                                            <Image source={getWeatherImage(forecast?.day?.condition?.text)} className="h-12 w-12" />
                                            <Text className="text-white">{dayName}</Text>
                                            <Text className="text-white font-semibold text-xl">{forecast?.day?.avgtemp_c}°C</Text>
                                        </View>
                                    )
                                })}
                            </ScrollView>
